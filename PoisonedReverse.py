    ...
    for router in routers_and_tables:
        ...
        for r in range(m):#find min start
             for c in range(m): #the router's row to find min destination
                ...
                if cost == "-":
                    cost = float('inf')
                second_router, row = corresponding_router_and_row(router,r,c,router_names)
                find_min_table = routers_and_tables.get(second_router)
                find_min_row = copy.deepcopy(find_min_table[row])
                for i in range(len(find_min_row)):
                    if find_min_row[i] == "-":
                        find_min_row[i] = float('inf')
                third_r_2_idx = find_min_row.index(min(find_min_row))
                second_table_labels = copy.deepcopy(router_names)
                second_table_labels.remove(second_router)
                third_router = second_table_labels[third_r_2_idx]
                if third_router == router:
                    #second_hoop_table = copy.deepcopy(routers_and_tables.get(third_router))
                    table[r][c] = float('inf')
                else:
...
while True:
    ...
    while True:
        ...
        #prints initial tabels
        #routers_and_tables = initial_tables(link_changes,router_names,new_routers_and_tables)
